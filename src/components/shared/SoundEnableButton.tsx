...
export default function SoundEnableButton({ className = '', variant = 'light' }: Props) {
    ...
    if (justEnabled) {
        return (
            <span className={`flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-lg ${variant === 'dark' ? 'text-green-300' : 'text-green-600'} ${className}`}>
    ...
    return (
        <button
            ...
            className={`flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-lg border transition ${variant === 'dark' ? darkStyles : lightStyles} ${className}`}
