-- Phase 1.2 & 1.3: Dynamic pricing + ingredient deduction
-- Run these in the Supabase SQL Editor after deploying this version.

-- ============================================================
-- Phase 1.2: apply_pricing_rules_to_order
-- Called right after place_order succeeds.
-- Finds the highest-priority active rule for each line item
-- and updates order_items.unit_price + recalculates order totals.
-- ============================================================
...
DECLARE
    ...
    v_dow           INT         := EXTRACT(DOW FROM v_now)::INT; -- 0=Sun … 6=Sat
...
BEGIN
    ...
    LOOP
        -- Highest-priority rule: item-specific wins over category wins over all-items
        SELECT *
        INTO   v_rule
        FROM   pricing_rules
    ...
    -- Recalculate order totals (preserve existing discount and tax)
    UPDATE orders
    SET
...
-- ============================================================
-- Phase 1.3: deduct_ingredients_for_order
-- Called right after apply_pricing_rules_to_order.
-- Reads recipes for each ordered item and subtracts quantities
-- from ingredients.stock_quantity, then logs ingredient_movements.
-- ============================================================
...
-- Useful diagnostic: see active rules for a restaurant right now
-- SELECT * FROM pricing_rules
-- WHERE restaurant_id = '<id>' AND is_active = true
-- AND EXTRACT(DOW FROM now())::INT = ANY(days_of_week)
-- AND now()::TIME BETWEEN start_time::TIME AND end_time::TIME;
