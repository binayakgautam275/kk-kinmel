-- ─────────────────────────────────────────────────────────────────────────────
-- Fix: "Sending to Kitchen" duplicate-order race condition
--
-- A customer double-tapping "Send to Kitchen" (or a network retry / second tab)
-- could fire place_order twice and create two identical orders on the kitchen
-- queue. The session FOR UPDATE lock serialized the two calls but did not dedupe
-- them, so both committed.
--
-- This migration makes order placement idempotent via a caller-supplied
-- client_request_id:
--   1. A partial UNIQUE index guarantees at most one order per request id.
--   2. place_order checks for an existing order with the same id (under the
--      session lock, so concurrent calls are serialized) and returns it instead
...
-- Recreate place_order (full overload) with idempotency support.
-- Drop the old 6-arg signature first; the new one adds p_client_request_id.
DROP FUNCTION IF EXISTS public.place_order(uuid, jsonb, text, uuid, text, uuid);
